...
def slide(direction):
    ...
    if direction == "down":
        ...
        for tiles in column1:
            if Combin:
                if lasttile[2] == tiles[2] and lasttile[0] == tiles[0]:
                    column1.remove(tiles)
                    column1.remove(lasttile)
                    TileErase(tiles[0], tiles[1], tiles[2])
                    TileErase(lasttile[0], lasttile[1], lasttile[2])
                    column1.append([tiles[0], tiles[1], (tiles[2] + lasttile[2])])
                    TileDraw(tiles[0], tiles[1], (tiles[2] + lasttile[2]))
                    Combin = False
                lasttile = tiles
        column1.reverse()
        for tiles in column1:
